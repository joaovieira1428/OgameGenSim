...
{
    private SimCombatInformation? _cleanData;
    ...
    public async Task GlobalSetupAsync()
    {
        ...
        _cleanData = await loader.LoadCombatInformation(attackers, defenders);
    }
    ...
    public void DoBattle()
    {
        if (_cleanData == null)
        {
            throw new InvalidOperationException("Benchmark setup not completed.");
        }

        var battle = new Battle(_cleanData.Universe.Debrifactor, _cleanData.Universe.DefenseDebrisFactor, _cleanData.Universe.DeuteriumOnDebris);

        battle.DoBattle(_cleanData);
    }
}